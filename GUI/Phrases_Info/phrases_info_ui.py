...
# Necesarios para reusar las imágenes de los botones
def relative_to_assets_letters_information(path: str) -> Path:
    return assets_letters_information_path / Path(path)

# Función para crear la ventana con la información de una frase espécifica

def create_phrases_information_window(window, actual_phrase):
    ...
    # Crear un rectángulo para mostrar la imagen de la frase (extraída del JSON "phrase_path")
    ...
    # Crear un rectángulo decorativo para el encabezado del texto (Dira que es una "frase"
    canvas.create_rectangle(
        28.0, 68.0, 220.0, 169.0, fill="#F6AA1C", outline="")
    ...
    from GUI.gui_utils import go_dictionary_phrases_window
    ...
    from GUI.gui_utils import go_camera_phrases_window

    # Configurar la lógica del botón "Regresar" (detiene la animación y vuelve al diccionario del alfabeto)
    button_go_back.config(command=lambda: (stop_letter_gif_animation(window), go_dictionary_phrases_window(window)))
    ...
    button_go_camera.config(command=lambda: (stop_letter_gif_animation(window), go_camera_phrases_window(window, actual_phrase)))
