...
# Inicializar el modelo estático TFLite
...
def start_letters_complete_recognition_cycle(window, video_label, progress_label, current_letter_index=0):
    """Inicia el ciclo de reconocimiento de señas y actualiza la interfaz con los resultados,
    utilizando el modelo estático (TFLite) y el modelo dinámico (TFLite)."""
    ...
    def update_frame():
        ...
        if current_letter_index >= len(ALPHABET):
            # Al terminar la última letra, muestra el popup y reinicia la lección
        ...
        if not hand_detected:
            ...
            if movement_type == "Estatico":
                ...
                with mp_hands_solution.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.5) as hands:
                    ...
                    if results_hands.multi_hand_landmarks:
                        for hand_landmarks in results_hands.multi_hand_landmarks:
                            ...
                            if confidence > 0.7 and predicted_letter == actual_letter and not transitioning:
                                transitioning = True
                                ...
                                last_correct_prediction = None
                            update_ui_on_prediction(window, predicted_letter if confidence > 0.7 else "low_confidence",
                                                    actual_letter)
                    else:
                ...
                if count_frame >= MIN_LENGTH_FRAMES + 1:
                    ...
                    if fix_frames >= 3:
                        ...
                        # Realizar predicción con el modelo dinámico TFLite
...
def restart_lesson(window, progress_label):
    ...
    start_letters_complete_recognition_cycle(window, progress_label)
