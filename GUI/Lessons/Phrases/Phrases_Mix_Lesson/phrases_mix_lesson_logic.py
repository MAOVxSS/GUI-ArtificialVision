...
# Seleccionar 5 frases aleatorias de PHRASES
SELECTED_PHRASES = random.sample(PHRASES, 5)
...
def start_phrases_mix_recognition_cycle(window, video_label, progress_label, current_letter_index=0):
    """Inicia el ciclo de reconocimiento de señas con frases seleccionadas aleatoriamente y actualiza la interfaz.
            ...
            if not transitioning:
                if count_frame >= MIN_LENGTH_FRAMES + 1:
                    ...
                    if fix_frames >= 3:
                        ...
                        kp_normalized = normalize_keypoints(keypoint_sequence, MODEL_FRAMES)
                        ...
                        if confidence > 50:
                            ...
                            if general_word == actual_phrase_lower and not transitioning:
                                ...
                                correct_count += 1
                                progress_label.config(text=f"{correct_count}/5")
                                window.after(2000, transition_to_next_phrase)
                            else:
    ...
    def transition_to_next_phrase():
        nonlocal current_letter_index, last_correct_prediction, transitioning
        current_letter_index = next_phrase(window, video_label, current_letter_index, progress_label)
        if current_letter_index < len(SELECTED_PHRASES):
            last_correct_prediction = SELECTED_PHRASES[current_letter_index]
        transitioning = False

    # Inicializar la primera frase y actualizar el icono
    actual_phrase = SELECTED_PHRASES[current_letter_index]
    phrase_data = load_phrase_data(actual_phrase)
    if phrase_data:
...
def next_phrase(window, video_label, current_letter_index, progress_label):
    current_letter_index += 1
    if current_letter_index >= len(SELECTED_PHRASES):
        # La finalización se controla en update_frame
...
def restart_lesson(window, video_label, progress_label):
    """Reinicia la lección, restableciendo el progreso y comenzando el ciclo de reconocimiento."""
    global SELECTED_PHRASES
    SELECTED_PHRASES = random.sample(PHRASES, 5)
    random.shuffle(SELECTED_PHRASES)
    if isinstance(progress_label, Label):
        progress_label.config(text="0/5")
    start_phrases_mix_recognition_cycle(window, video_label, progress_label)
