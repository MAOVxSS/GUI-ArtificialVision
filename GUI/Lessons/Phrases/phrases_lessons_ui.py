...
# Rutas
from Utils.paths import assets_lessons_alphabet_path, assets_phrases_lessons_path
from GUI.gui_utils import setup_window, create_common_canvas, BUTTON_COMMON_CONFIG


def relative_to_alphabet_lessons_assets(path: str) -> Path:
    return assets_lessons_alphabet_path / Path(path)

def relative_to_phrases_lessons_assets(path: str) -> Path:
    return assets_phrases_lessons_path / Path(path)

def create_phrases_lessons_window(window):
    ...
    button_image_home = PhotoImage(file=relative_to_alphabet_lessons_assets("inicio.png"))
    ...
    button_image_go_back = PhotoImage(file=relative_to_alphabet_lessons_assets("regresar.png"))
    ...
    button_image_mix_lesson = PhotoImage(file=relative_to_alphabet_lessons_assets("leccion_azar.png"))
    ...
    button_image_complete_lesson = PhotoImage(file=relative_to_phrases_lessons_assets("button_4.png"))
    images["button_4"] = button_image_complete_lesson
    ...
    image_image_1 = PhotoImage(file=relative_to_phrases_lessons_assets("image_1.png"))
    images["image_1"] = image_image_1
    canvas.create_image(979.0, 127.0, image=image_image_1)

    image_image_2 = PhotoImage(file=relative_to_phrases_lessons_assets("image_2.png"))
    ...
    from GUI.gui_utils import go_lessons_window, go_home_window, go_complete_lesson_phrases, go_mix_lesson_phrases
    ...
    button_complete_lesson.config(command=lambda: go_complete_lesson_phrases(window))

    # Lógica del botón "Lección mix"
    button_mix_lesson.config(command=lambda: go_mix_lesson_phrases(window))
